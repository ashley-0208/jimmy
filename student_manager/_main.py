...
from _login_window import LoginWindow

root = Tk()
root.withdraw()  # used to hide the main window of a GUI app
...
# Welcome
welcome_label = tkinter.Label(root, text=f"logged in as: {getattr(root, "username", "Guest")}",
...
Label(search_frame, text="Search:").grid(row=0, column=0, padx=5)
...
def search_student():
    ...
    if not query:
        messagebox.showerror("Search", "Please enter a term.")
        return
    ...
    try:
        with get_conn() as conn:
            ...
            cursor.execute("""
                ...
                WHERE FullName LIKE ? OR Major LIKE ?
            """, (f"%{query}%", f"%{query}%"))
...
btn_search = Button(search_frame, text="Search", command=lambda: search_student())
